module CursorPaginatable
  # https://radanskoric.com/guest-articles/pagy-out-turbo-in
  def paginate_with_cursor(relation, items: 20, before: nil, by: :id, dir: :desc)
    # Filter by cursor start value, if one is provided. If missing, we know
    # we're on the first page.
    if before.present?
      relation = relation.where(
        dir == :desc ? { by => ..before } : { by => before.. },
      )
    end

    # Order the relation by the cursor field, and limit it to
    # `items + 1` records.
    # This is because we want to know if there are more records to load,
    # and we need to know that before we actually load them.
    relation = relation.reorder(by => dir).limit(items + 1).to_a

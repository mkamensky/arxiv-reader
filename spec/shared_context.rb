...
RSpec.shared_context 'general context' do |typ = find_model_type|
  typ.to_s.classify.constantize
    .reflect_on_all_associations(:belongs_to).each do |ass|
    meth = ass.name.to_s == 'subject' ? '_subject' : ass.name
    let(meth) { create(ass.name) }
  end
  let(:type) { typ.to_s }
  ...
  let(:payload) do
    {
      data: {
        ...
        relationships: Hash[relationships.map do |rel|
          meth = rel.name.to_s == 'subject' ? '_subject' : rel.name
          [rel.name, data: {
            type: rel.name.to_s.pluralize,
            id: send(meth).id.to_s,
          }]
        end],
      },
    }
...
RSpec.shared_context 'update context' do
  ...
  let(:update_object) do
    ...
    update_params.reduce(change { object.reload.updated_at }) do |res, (k, v)|
      res.and(change { object.send(k) }.to(v))
    end
  end
end
